...
from SourceControlMgmt.SourceControlMgmt import (SCMCredentialValidationError, SCMCloneRepoError,
                                                 ...
                                                 SCMPushDataError, SCMDeleteRepoError, SCMGraphQLError)
...
def mock_requests():
    ...
    class Mock_Requests_Return_Fail():
        def json(self):
            return {
                'data': {
                    'errors': {
                        'fake_test_error': 'test is well'
                    }
                }
            }
    ...
    class Mock_Requests():
        def mock_post(*args, **kwargs):
            if 'fail' in kwargs['json']['query']:
                return Mock_Requests_Return_Fail()
            else:
                return Mock_Requests_Return()
...
@pytest.fixture
def scm(setup, monkeypatch, mock_requests):
    monkeypatch.setattr(requests, "post", mock_requests.mock_post)
...
def test_create_object_no_parameters(monkeypatch, mock_requests):
...
def test_create_object_missing_password(setup, monkeypatch, mock_requests):
...
def test_gql_query_function(scm):
    with pytest.raises(TypeError) as e:
        scm._gql_query()

    assert str(e.value) == "A GraphQL query is required to run this function"


def test_gql_query_with_errors(scm):
    expected_failure_message = "An error in GraphQL occured.  See the following for more info: {'fake_test_error': 'test is well'}"

    with pytest.raises(SCMGraphQLError) as e:
        scm._gql_query(query="please fail")

    assert str(e.value) == expected_failure_message


def test_create_git_hub_pull_request_no_parameters(scm):
    with pytest.raises(TypeError) as e:
        scm.create_git_hub_pull_request()

    assert str(e.value) == "Must have a source and destination branch to create a Pull Request"


def test_create_git_hub_pull_request(scm):
    results = scm.create_git_hub_pull_request(source_branch='src', destination_branch='dest')

    assert isinstance(results, dict)


def test_get_all_current_branches(scm):

    assert not scm.existing_branches
    scm.get_all_current_branches()

    assert scm.existing_branches
    assert isinstance(scm.existing_branches, dict)
    assert scm.existing_branches == {'fake_name': 'fake_id'}
