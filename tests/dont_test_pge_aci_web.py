...
@pytest.fixture
def client():
    pge_aci_web.app.config['TESTING'] = True
    pge_aci_web.app.config['SECRET_KEY'] = 'fakekey'
    pge_aci_web.app_context = pge_aci_web.app.test_request_context()
    pge_aci_web.app_context.push()
    client = pge_aci_web.app.test_client()
...
@pytest.fixture
def app():
    with pge_aci_web.app.app_context() as app:
        yield app
...
def test_test_for_login_info_no_login_info(client):
    with client as c:
        ...
        rv = pge_aci_web.test_for_login_info()
        assert rv is False
...
def test_clean_string_matching_data():
    rv = pge_aci_web.clean_string('(test_data)')
    assert isinstance(rv, str)
    assert rv == 'test_data'


def test_clean_string_no_matching_data():
    rv = pge_aci_web.clean_string('non_matching_test_data')
    assert isinstance(rv, str)
    assert rv == ''


def test_clean_string_int_search_data():
    rv = pge_aci_web.clean_string(42)
    assert isinstance(rv, str)
    assert rv == ''
...
def test_clean_data_matching_data_with_valid_data(client):
    ...
    with client as c:
        ...
        rv = pge_aci_web.clean_data(request, 'my_test')
...
<!-- End document_header.html -->'''  # NOQA - some line causes flake8 issues
    ...
    with client:
        rv = pge_aci_web.document_header()
...
def test_document_footer(client):
    ...
    with client:
        rv = pge_aci_web.document_footer()
        assert rv == expected_results
...
def test_render_error_with_string(client):
    ...
    with client:
        rv = pge_aci_web.render_error_screen(error_message)
...
def test_basic_render_with_index_html(client):
    ...
    with client:
        rv = pge_aci_web.basic_render(data_url, page_header, rendering_file, form_data)
...
def test_get_fabric_friendly_name_no_fabric_names_defined(client):
    ...
    with client:
        rv = pge_aci_web.get_fabric_friendly_name(test_url)
        assert rv == test_url
...
def test_get_fabric_friendly_name_fabric_names_defined(client):
    ...
    pge_aci_web.app.settings.fabric_names = {
        'http://www.fake.com': "Fake Fabric Name"
    }

    with client:
        rv = pge_aci_web.get_fabric_friendly_name(test_url)
        assert rv == pge_aci_web.app.settings.fabric_names[test_url]


def test_base_menu_no_creds(client):
    pge_aci_web.creds = {}
    with client:
        rv = pge_aci_web.base_menu()
...
def test_base_menu_with_creds(client):
    pge_aci_web.creds = {"apic": "256.2.3.4"}
...
def test_save_addepg_post_method_with_data(app, client, monkeypatch):
    ...
    monkeypatch.setattr("pge_aci_web.write_data_to_git_repo", mockreturn)
...
def test_render_login_screen(client):
    with client:
        rv = pge_aci_web.render_login_screen()
        print(rv)
        print(dir(rv))
        assert rv.status_code == 303
...
def test_get_url_fake_good_url_fail_to_load_data_from_server(mock_request, client):
    ...
    with client:
        rv = pge_aci_web.get_url(test_url)
        assert rv['result'] == 0
...
def test_source_control_credential_input(client):
    rv = pge_aci_web.source_control_credential_input()
    assert isinstance(rv, str)
    assert "SCM" in rv
...
def test_write_data_to_git_repo_no_data(client):
    with client:
        with pytest.raises(TypeError) as e:
            pge_aci_web.write_data_to_git_repo()
...
def test_write_data_to_git_repo(app, client, monkeypatch, MockSCM):
    monkeypatch.setattr("pge_aci_web.SourceControlMgmt.SourceControlMgmt", MockSCM)
    with client:
        pge_aci_web.write_data_to_git_repo()
...
def test_write_data_to_git_repo_an_error_occurs(app, client, monkeypatch, MockSCM):
    ...
    monkeypatch.setattr("pge_aci_web.SourceControlMgmt.SourceControlMgmt", m_scm)
    with client:
        pge_aci_web.write_data_to_git_repo()
        assert session.get('message', '') == expected_return
...
def test_index_page_message_default_values():
    ...
    rv = pge_aci_web.index_page_message()
...
def test_index_page_message_unknown_success_message():
    ...
    rv = pge_aci_web.index_page_message(success="invalid")
    assert rv == expected_return
...
def test_index_page_message_success():
    ...
    rv = pge_aci_web.index_page_message(msg=success_message, success=True, url=url)
...
def test_index_page_message_fail():
    ...
    rv = pge_aci_web.index_page_message(success=False, msg=fail_message)
